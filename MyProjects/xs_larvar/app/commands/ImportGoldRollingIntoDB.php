...
class ImportGoldRollingIntoDB extends Command {

	const TABLE_NAME = 'log_gold_rolling';
	const FILE_KEY = 'GoldRollingLog';
	const PREFIX_LOG_FILE = '/home/game/trans/data/';
	const LOG_BACKUP = '/data/logbackup/';
	const FLAG_TYPE = 'gold_rolling';
	...
	private function createTable(){
	    ...
	    $sql="CREATE TABLE IF NOT EXISTS `log_gold_rolling`(
	        ...
	        `cost_yuanbao` int(10) unsigned NOT NULL,
	        `get_yuanbao` int(10) unsigned NOT NULL,
	         INDEX time_playerid (`time`,`player_id`)
	        )ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1";
	    mysqli_query($con,$sql);
	}

	protected $name = 'goldRolling:import';
	 ...
	 * @var string
	 */
	protected $description = 'Import goldRolling Log.';
	...
	private function loadGmaeFile()
	{
	    ...
	    try
	    {
	        $result = $this->db->select("show tables like 'log_gold_rolling'");
	        if(!$result){
	            $this->createTable();
	        }  
	    }
	    ...
	    Log::info('Import ' . $this->db_name . ' goldRolling Log');
	    try
	    {
	    ...
	    if (! $files)
	    {
	        Log::info($this->db_name . ' goldRolling Log Not Found');
	        return;
	    }
