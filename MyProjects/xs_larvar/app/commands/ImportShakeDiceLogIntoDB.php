...
class ImportShakeDiceLogIntoDB extends Command {

	const TABLE_NAME = 'log_shake_dice';
	const FILE_KEY = 'ShakeDiceLog';
	const PREFIX_LOG_FILE = '/home/game/trans/data/';
	const LOG_BACKUP = '/data/logbackup/';
	const FLAG_TYPE = 'shake_dice';
	...
	private function createTable(){
	    ...
	    $sql="CREATE TABLE IF NOT EXISTS `log_shake_dice`(
	    	...
	    	`win_number` smallint(4) unsigned NOT NULL,
	    	`lose_number` smallint(4) unsigned NOT NULL,
	        `store_back` int(10) unsigned NOT NULL,
	         INDEX time_playerid (`time`,`player_id`)
	        )ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1";
	    mysqli_query($con,$sql);
	}

	protected $name = 'shakeDice:import';
	 ...
	 * @var string
	 */
	protected $description = 'Import ShakeDice Log.';
	...
	private function loadGmaeFile()
	{
	    ...
	    try
	    {
	        $result = $this->db->select("show tables like 'log_shake_dice'");
	        if(!$result){
	            $this->createTable();
	        }  
	    }
	    ...
	    Log::info('Import ' . $this->db_name . ' ShakeDice Log');
	    try
	    {
	    ...
	    if (! $files)
	    {
	        Log::info($this->db_name . ' ShakeDice Log Not Found');
	        return;
	    }
