...
function getUserStatController($scope, $http, alertService, $filter) {
    ...
    $scope.download = function() {
    	$scope.formData.tobedownload = $scope.items;
        $http({
            'method': 'post',
            'url': '/slave-api/user/stat/download',
            'data': $.param($scope.formData),
            'headers': {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
            ...
            window.location.replace("/slave-api/user/stat/download?now=" + data.now);
        }).error(function(data) {
            alertService.add('danger', data.error);
        });
    };
	...
	<div class="row">
		<div class="eb-content">
			<form action="/slave-api/user/userstat" method="get" role="form"
				...
				<input type='button' class="btn btn-warning" style="margin-left:300px;"
		...
		<table class="table table-striped">
			...
			<tbody>
				<tr ng-repeat="t in items">
					...
					<td><div ng-if="t.count_player_formal!=null">{{t.count_player_formal}}</div></td>
					<td><div ng-if="t.count_player_formal!=null">{{t.count_player_formal/t.count_formal*100| number:2}}%</div></td>
					<td><div ng-if="t.count_player_anonymous!=null">{{t.count_player_anonymous}}</div></td>
					<td><div ng-if="t.count_player_anonymous!=null">{{t.count_player_anonymous/t.count_anonymous*100| number:2}}%</div></td>
					<td><div ng-if="t.count_lev_formal!=null">{{t.count_lev_formal}}</div></td>
					<td><div ng-if="t.count_lev_formal!=null">{{t.count_lev_formal/t.count_player_formal*100| number:2}}%</div></td>
					<td><div ng-if="t.count_lev_anonymous!=null">{{t.count_lev_anonymous}}</div></td>
					<td><div ng-if="t.count_lev_anonymous!=null">{{t.count_lev_anonymous/t.count_player_anonymous*100| number:2}}%</div></td>
				</tr>
