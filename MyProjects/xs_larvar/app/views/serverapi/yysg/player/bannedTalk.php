<script>
	function bannedTalkController($scope, $http, alertService, $filter) {
		$scope.alerts = [];
		$scope.formData = {};
		$scope.auto = 0;
		...
		$scope.auto_process = function() {
			...
			$http({
				'method' : 'post',
				'url'	 : '/game-server-api/yysg/bannedTalk',
				'data'   : $.param($scope.formData),
				'headers': {'Content-Type' : 'application/x-www-form-urlencoded'}
			}).success(function(data) {
				$scope.items = data;
		...
		<div class="eb-content">
			<div class="form-group">
				<select class="form-control" name="is_banned" ng-model="formData.is_banned" ng-init="formData.is_banned=1">
					<option value="1"><?php echo Lang::get('serverapi.select_banner');?></option>
			...
			<div class="form-group" ng-if="formData.is_banned==1">
				<select class="form-control" name="ban_time" ng-model="formData.ban_time" ng-init="formData.ban_time=0">
					<option value="0">永久禁言(需要手动解封)</option>
            ...
            <div class="form-group">
                <input type="text" class="form-control" id="close_reason"
                       placeholder="<?php echo Lang::get('serverapi.enter_banned_reason') ?>"
                       required ng-model="formData.close_reason" name="close_reason" />
            </div>

			<input type="submit" class="btn btn-warning"
				ng-click="processFrom('/game-server-api/yysg/bannedTalk')" value="<?php echo Lang::get('basic.btn_submit') ?>" />
			...
			<div class="form-group">
			    ...
			    <div class="col-md-2">
			    	...
			    	ng-click="searchFrom('/game-server-api/yysg/bannedTalk')"
