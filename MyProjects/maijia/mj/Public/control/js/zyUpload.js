		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '		<div class="upload_main" style="width: 89%;">';
					html += '			<div class="upload_choose" style="height: 50px;">';
	            	html += '				<div class="convent_choice"  style="height: 50px;">';
	            	html += '					<div class="" >';
	            	html += '						<div class="filePicker text-center" style="margin-left: 100%;">点击选择文件</div>';
	            	html += '						<input id="fileImage" type="file" size="40"  name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '			</div>';
					html += '			<div id="preview" style="width: 100%;" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf"></div>';
					html += '<div class="col-lg-offset-4 col-lg-4" style="margin-top: 50px;;">';
					html += '	<button class="btn btn-sm btn-success" type="submit">提交</button>';
					html += '</div>';
					html += '	</div>';
				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var wid=parseInt(imgWidth)+parseInt(15);
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '			<img id="uploadImage_'+file.index+'" class="upload_iage" src="' + e.target.result + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
					html += '		</div>';
					html += '	</a>';
					html += '		<p data-toggle="modal" data-target="#myModal" class="img_depict_show" id="img_depict_show'+file.index+'" style="color: #f0ad4e; display: none;  overflow: hidden; white-space: nowrap;   text-overflow: ellipsis; border: #eeeeee 2px solid; width:'+wid+'px;"></p>';
					html += '		<input name="depict[]" class="img_depict_text" type="hidden" style="width:'+100+'%;">';
					html += '		<p data-toggle="modal" data-target="#myModal" style="text-align: center; color: #f0ad4e; width: 100%; border: #eeeeee 2px solid;" class="img_depict" id="img_depict_'+file.index+'" >添加描述</p>';
