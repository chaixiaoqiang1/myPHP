            ...
            <div class=" page-content clearfix" style="width: 1700px">
                <div id="products_style">
                    <div class="border clearfix">
                            <span class="l_f">
                            <a href="<?php echo U('Comment/add');?>" title="添加图片" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加图片</a>
                           </span>
                    ...
                    <div class="h_products_list clearfix" id="products_list">
                        ...
                        <div class="table_menu_list" id="wd">
                            <table class="table table-striped table-bordered table-hover" id="sample-table" >
                                ...
                                <?php if(is_array($comment)): foreach($comment as $key=>$v): ?><tbody>
                                    <tr>
                                        <td width="10%"><?php echo ($v["id"]); ?></td>
                                        <?php switch($v["type"]): case "1": ?><td>颜值与设计</td><?php break;?>
                                            <?php case "2": ?><td>驾驶与操控</td><?php break;?>
                                            <?php case "3": ?><td>舒适便利性</td><?php break;?>
                                            <?php case "4": ?><td>避险能力</td><?php break;?>
                                            <?php case "5": ?><td>抗损耐用性</td><?php break;?>
                                            <?php case "6": ?><td>通过性(SUV)</td><?php break;?>
                                            <?php default: ?>其他<?php endswitch;?>
                                        <td width="15%"> <img src="/maijia/<?php echo ($v["img"]); ?>"  style=" height: 100px; width: 100%;"/></td>
                                        <td width="5%"><?php echo ($v["size"]); ?></td>
                                        <td width="5%"><?php echo ($v["time"]); ?></td>
                                        ...
                                        <?php else: ?>
                                            ...
                                            <td class="td-manage">
                                                <a style="text-decoration:none" class="btn btn-xs " onClick="member_start(this,'<?php echo ($v["id"]); ?>')" href="javascript:;" title="启用"><i class="fa fa-close bigger-120"></i></a><?php endif; ?>
                                            <a title="编辑"  href="/maijia/index.php/Admin/Comment/editor/id/<?php echo ($v["id"]); ?>"
                                ...
                                <td colspan="7"><?php echo ($listShow); ?></td>
                            </table>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function member_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            $.ajax({
                type:"post",
                url:"/maijia/index.php/Admin/Comment/stop",
                data:"id="+id,
                dataType:"html",
                success:function (re){
                    if(re==1){
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type:"post",
                url:"/maijia/index.php/Admin/Comment/delete",
                data:"id="+id,
                dataType:"html",
                success:function(re){
                    if(re=="OK"){
