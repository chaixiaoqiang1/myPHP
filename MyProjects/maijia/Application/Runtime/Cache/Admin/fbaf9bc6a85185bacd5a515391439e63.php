        ...
        <div class="main-content">
            ...
            <div class="breadcrumbs" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li class="active"><span class="Current_page iframeurl">课程列表</span></li>
            ...
            <div class=" page-content clearfix" style="width: 1700px">
                <div id="products_style">
                    <div class="search_style" style="height: 80px">
                        ...
                        <form action="<?php echo U('Question/course');?>" method="post" id="form01">
                            <ul class="search_content clearfix">
                                <li><label class="l_f">标题</label><input name="title" type="text" class="text_add"
                                                                        placeholder="输入标题关键字"
                                                                        style=" width:250px"/></li>
                                <li>
                                    <label class="l_f" style="margin-right: 10px;">分类</label>
                                    <select name="course_type" id="">
                                        <option value="0">请选择分类</option>
                                        <option value="1">青葱课</option>
                                        <option value="2">政治课</option>
                                        <option value="3">物理课</option>
                                        <option value="4">是非课</option>
                                        <option value="5">魔法课</option>
                                    </select>
                                </li>
                                <li><label class="l_f">添加时间</label><input class="inline laydate-icon" id="start"
                                                                          name="time" style=" margin-left:10px;"></li>
                                <li style="width:90px;">
                    ...
                    <div class="border clearfix">
                            <span class="l_f">
                            <a href="<?php echo U('Question/add_course');?>" title="添加课程" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加课程</a>
                            <a href="javascript:;" class="btn btn-danger" id="piliang"><i class="icon-trash"></i>批量删除</a>
                           </span>
                    ...
                    <div class="h_products_list clearfix" id="products_list">
                        ...
                        <div class="table_menu_list" id="wd">
                            <table class="table table-striped table-bordered table-hover" id="sample-table" >
                                <thead>
                                    <tr>
                                        <th width="5%"><label><input type="checkbox" class="ace" id="ace_one"><span class="lbl"></span></label>
                                        </th>
                                        <th width="5%">所属分类</th>
                                        <th width="20%">标题</th>
                                        <th width="10%">分享数</th>
                                ...
                                <form action="<?php echo U('Question/del_list');?>" method="post" id="form03">
                                <tbody>
                                <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        <td width="5%"><label><input type="checkbox" class="ace" name="dellist[]" value="<?php echo ($vo["id"]); ?>"><span class="lbl"></span></label>
                                        </td>
                                        <?php if($vo['course_type'] == 1): ?><td width="5%">青葱课</td>
                                            <?php elseif($vo['course_type'] == 2): ?>
                                            <td width="5%">政治课</td>
                                            <?php elseif($vo['course_type'] == 3): ?>
                                            <td width="5%">物理课</td>
                                            <?php elseif($vo['course_type'] == 4): ?>
                                            <td width="5%">是非课</td>
                                            <?php elseif($vo['course_type'] == 5): ?>
                                            <td width="5%">魔法课</td>
                                            <?php else: ?>
                                            <td width="5%"></td><?php endif; ?>
                                        <td width="20%"><?php echo ($vo["title"]); ?></td>
                                        <td width="10%"><?php echo ($vo["share"]); ?></td>
                                        <td width="10%"><?php echo ($vo["praise"]); ?></td>
                                        <td width="10%"><?php echo ($vo["message"]); ?></td>
                                        <td width="10%"><?php echo ($vo['time']); ?></td>
                                   <?php if($vo['status'] == 1): ?><td class="td-status"><span class="label label-success radius">已启用</span></td>
                                       <?php else: ?>
                                       <td class="td-status"><span class="label label-defaunt radius">已关闭</span></td><?php endif; ?>

                                        <td class="td-manage">
                                            <a title="查看留言" href="<?php echo U('Question/course_com',array('id'=>$vo['id']));?>"
                                                                  class="btn btn-xs btn-warning"><i class="icon-comment  bigger-120"></i></a>
                                            <?php if($vo['status'] == 1): ?><a onClick="member_stop(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;" title="停用"
                                          ...
                                          <!--  <a title="编辑" onclick="member_edit('编辑','edit_course.html','4','','510')" href="add_content.html"
                                               class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i></a>-->
                                            <a title="编辑" href="<?php echo U('Question/edit_course',array('id'=> $vo['id']));?>" class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i></a>
                                            ...
                                            <a title="查看详情" href="<?php echo U('Question/see',array('id'=> $vo['id']));?>"
                                               class="btn btn-xs btn-success"><i class="icon-eye-open  bigger-120"></i></a>
                                        </td>
                                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                                </tbody>
                                </form>
                            </table>
                            <div id="page" class="text-center" style="margin-top: 5px;">
                                <?php echo ($page); ?>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function member_stop(obj, id) {
        layer.confirm('确认要停用吗？', function (index) {
            $.post("<?php echo U('Question/edit_course_status');?>",{'id':id},function(data){
                   if(data == 'ok'){
                       location.reload();
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type:"post",
                url:"/maijia/index.php/Admin/Question/del_course",
                data:"id="+id,
                dataType:"html",
                success:function(re) {
                    if(re=="ok") {
