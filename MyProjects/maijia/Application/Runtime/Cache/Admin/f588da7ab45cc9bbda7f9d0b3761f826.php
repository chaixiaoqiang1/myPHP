            ...
            <div class="breadcrumbs" id="breadcrumbs">
                <ul class="breadcrumb">
                    <li>
                        ...
                        <a href="<?php echo U('Banner/index');?>">车主培训</a>
                    </li>
            ...
            <div class=" clearfix" id="advertising">
                ...
                <div class="Ads_list" id="" style="width: 1730px">
                    <div class="border clearfix">
                       <span class="l_f">
                        <a href="<?php echo U('Train/add');?>" id="ads_add" class="btn btn-warning"><i class="fa fa-plus"></i> 添加培训</a>
                      ...
                      <table class="table table-striped table-bordered table-hover" id="sample-table">
                          ...
                          <form action="<?php echo U('Train/delete_all');?>" method="post">
                            <tbody>
                            <?php if(is_array($info)): $k = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                                ...
                                <td>
                                    <?php if($vo['type'] == 2): ?>觅驾车主测评制作约定
                                        <?php else: ?>
                                        觅车答疑/助驾服务规范<?php endif; ?>
                                </td>
                                <td><?php echo ($vo["title"]); ?></td>
                                <?php if($vo['status'] == 1): ?><td class="td-status"><span class="label label-success radius">显示</span></td>
                                    <td class="td-manage">
                                    ...
                                    <a title="编辑" href="<?php echo U('Train/edit',array('id'=> $vo['id']));?>" class="btn btn-xs btn-info"><i class="fa fa-edit bigger-120"></i></a>
                                ...
                                <a title="下载" download="<?php echo ($vo["title"]); ?>" href="/maijia/<?php echo ($vo["path"]); ?>"
...
<script>
    ...
    function member_stop(obj, id) {
        layer.confirm('确认要关闭吗？', {icon: 0,}, function (index) {
            $.post("<?php echo U('Train/edit_status');?>",{'id':id},function(data){
                if(data == 'ok'){
                    location.reload();
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', {icon: 0,}, function (index) {
            $.post("<?php echo U('Train/del');?>",{'id':id},function(data){
                if(data == 'ok'){
