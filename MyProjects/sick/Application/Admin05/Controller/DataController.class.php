...
//		var_dump($data,$show);die;
	...
	public function doctorconsult(){
		...
		setPage($Page);
		$show= $Page->show();
		$list = $this->model->where($where)->field('id,username,office,tuwenpirce,orderprice,imagetext_people,order_people,free_people,servernum,total_price')->order('office')->limit($Page->firstRow.','.$Page->listRows)->select();
		foreach($list as $k=>&$v){
