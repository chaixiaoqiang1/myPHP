            ...
            <div class="row">
                <table class="table table-bordered">
                    <tr class="text-center">
                        <td class="active">用户类型</td>
                        <td class="active">用户ID</td>
                        <td class="active">用户名</td>
                        <td class="active">支付宝账号</td>
                        <td class="active">余额</td>
                        <td class="active">提现金额</td>
                        <td class="active">状态</td>
                        <td class="active">操作</td>
                    </tr>
                    <?php if(is_array($info)): foreach($info as $k=>$vo): ?><tr class="text-center">
                            <td class="active"><?php if($vo['type'] == 1): ?>患者<?php elseif($vo['type'] == 2): ?>医生<?php endif; ?></td>
                            <td class="active">
                                <?php echo ($vo["userid"]["id"]); ?>
                            </td>
                            <td class="active"><?php echo ($vo["userid"]["username"]); ?></td>
                            <td class="active"><?php echo ($vo["account_number"]); ?></td>
                            <td class="active"><?php echo ($vo["userid"]["balance"]); ?>元</td>
                            <td class="active"><?php echo ($vo["price"]); ?>元</td>
                            <td class="active">
                                <?php if($vo['is_statu'] == 1): ?>未提现
                                    <?php elseif($vo['is_statu'] == 2): ?>已提现
                                    <?php elseif($vo['is_statu'] == 3): ?>驳回<?php endif; ?>
                            </td>
                            <td class="active">
                                <!-- <a href="<?php echo U('Admin/status',array('id'=>$vo['id']));?>">
                                     <span class="label btn-warning">冻结</span>
                                 </a>
                                 <a href="<?php echo U('Admin/status',array('id'=>$vo['id']));?>">
                                     <span class="label btn-success">正常</span>
                                 </a>-->
                                <?php if($vo['is_statu'] == 2): ?><a href="<?php echo U('Admin/setprice',array('id'=>$vo['id'],'type'=>3));?>">
                                        <span class="label btn-warning">删除</span>
                                    </a>
                                    <?php else: ?>
                                    <a href="<?php echo U('Admin/zhifu',array('id'=>$vo['id'],'type'=>1));?>">
                                        <span class="label btn-danger">支付</span>
                                    </a>

                                    <a href="<?php echo U('Admin/setprice',array('id'=>$vo['id'],'type'=>2));?>">
                                        <span class="label btn-info">驳回</span>
                                    </a>
                                    <a href="<?php echo U('Admin/setprice',array('id'=>$vo['id'],'type'=>3));?>">
                                        <span class="label btn-warning">删除</span>
                                    </a><?php endif; ?>
