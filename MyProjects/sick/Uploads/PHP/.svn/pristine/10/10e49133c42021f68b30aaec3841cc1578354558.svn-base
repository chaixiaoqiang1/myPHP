        ...
        <div class="border-bottom white-bg dashboard-header">
            <div>
                <div class="row">
                    ...
                    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6" style="margin-top: 20px;">
                        <a href="<?php echo U('User/addMain');?>">
            ...
            <div class="col-lg-12" style="border:1px solid #eeeeee; padding-top: 20px; margin-top: 10px;">
                <table class="table table-hover table-mail">
                    ...
                    <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr level="1" id="<?php echo ($v["id"]); ?>" pid="<?php echo ($v["pid"]); ?>">
                            <td id="<?php echo ($v["id"]); ?>">
                                <?php if($v['children']): ?><span class="open">+</span><?php endif; ?>
                            </td>
                            ...
                            <td>
                                <a href="<?php echo U('User/addChild',array('id'=>$v['id']));?>" class="label label-primary">添加子级</a>
                                <a href="<?php echo U('Category/editor',array('id'=>$v['id']));?>" class="label label-info">修改</a>
                                <a href="<?php echo U('Category/delete',array('id'=>$v['id']));?>" class="label label-danger">删除</a>
                            </td>
                        </tr>
                        <?php if($v.children): if(is_array($v["children"])): $i = 0; $__LIST__ = $v["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?><tr level="2"　id="<?php echo ($v1["id"]); ?>" pid="<?php echo ($v1["pid"]); ?>">
                                    <td id="<?php echo ($v1["id"]); ?>">
                                        <?php if($v1['children']): ?><span class="open">+</span><?php endif; ?>
                                    </td>
                                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($v1["cate_name"]); ?></td>
                                    <td>
                                        <?php if($v1['status'] == 1): ?><span class="label label-warning">正常</span>
                                            <?php else: ?>
                                            <span class="label label-primary">冻结</span><?php endif; ?>
                                    </td>
                                    <td>
                                        <a href="<?php echo U('User/addChild',array('id'=>$v1['id']));?>" class="label label-primary">添加子级</a>
                                        <a href="<?php echo U('Category/editor',array('id'=>$v1['id']));?>" class="label label-info">修改</a>
                                        <a href="<?php echo U('Category/delete',array('id'=>$v1['id']));?>" class="label label-danger">删除</a>
                                    </td>
                                </tr>
                                <?php if($v1[children]): if(is_array($v1["children"])): $i = 0; $__LIST__ = $v1["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($i % 2 );++$i;?><tr level="3" id="<?php echo ($v2["id"]); ?>" pid="<?php echo ($v2["pid"]); ?>">
                                            <td>
                                            </td>
                                            <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($v2["cat_name"]); ?></td>
                                            <td>
                                                <?php if($v2['status'] == 1): ?><span class="label label-warning">正常</span>
                                                    <?php else: ?>
                                                    <span class="label label-primary">冻结</span><?php endif; ?>
                                            </td>
                                            <td>
                                                <a href="<?php echo U('Category/editor',array('id'=>$v2['id']));?>" class="label label-info">修改</a>
                                                <a href="<?php echo U('Category/delete',array('id'=>$v2['id']));?>" class="label label-danger">删除</a>
                                            </td>
