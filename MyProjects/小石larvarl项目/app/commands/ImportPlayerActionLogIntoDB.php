...
class ImportPlayerActionLogIntoDB extends Command {

    const TABLE_NAME = 'log_player_action';

    const FILE_KEY = 'PlayerActionLog';

    const PREFIX_LOG_FILE = '/home/game/trans/data/';

    const LOG_BACKUP = '/data/logbackup/';

    const FLAG_TYPE = 'playerAction';
     ...
     * @var string
     */
    protected $description = 'Import Player Action Log.';
    ...
    private function loadActionFile()
    {
        Log::info('Import ' . $this->db_name . ' Player Action Log');
        try
        {
        ...
        if (! $files)
        {
            Log::info($this->db_name . ' Player Action Log Not Found');
            return;
        }
        $this->action_files = array();
        foreach ($files as $v)
        {
            ...
            if (strpos($v, self::FILE_KEY) !== false)
            {
                ...
                $this->action_files[$tmp[1]] = $v;
            }
        }
        ksort($this->action_files);
    ...
    {
        while (! empty($this->action_files))
        {
            $pos = - 1;
            $v = array_shift($this->action_files);
            Log::info('Import ' . $v);
            ...
            // server_id,room_id,rule_id,period,player_id,buyin_chips,chips,private_cards,action,is_auto,time
            $i = 0;
            while ($log = fscanf($handle, 
