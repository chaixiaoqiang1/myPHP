...
class ImportGameLogIntoDB extends Command {

    const TABLE_NAME = 'log_game';

    const FILE_KEY = 'GameLog';

    const PREFIX_LOG_FILE = '/home/game/trans/data/';

    const LOG_BACKUP = '/data/logbackup/';

    const FLAG_TYPE = 'game';
    ...
    private function loadGameFile()
    {
        Log::info('Import ' . $this->db_name . ' Game Log');
        try
        {
        ...
        if (! $files)
        {
            Log::info($this->db_name . ' Game Log Not Found');
            return;
        }
