...
class PlatformDailyReport extends Command {
	//脚本目前没有再用
	...
	private $game_lists = array(
		38, 58, 46, 53, 67, 51, 52, 55, 64, 65, 71, 77
	);
	 ...
	 * @var string
	 */
	protected $name = 'platformdaily:report';
	 ...
	 * @var string
	 */
	protected $description = 'PlatformDaily Report.';
	...
	public function fire()
	{
		...
		$this->setDB();
		$game_ids = DB::connection($this->db_qiqiwu)->select("select game_id from game_list");
		foreach ($game_ids as $game_id_std) {
			$game_id = $game_id_std->game_id;
			...
			if(isset($this->subject_list[$game_id])){
				...
				$this->subject_list[$game_id] = $value->game_name.'日报';
			}
			$this->sendReport($game_id);
		}

    }

    private function sendReport($game_id){
		if(in_array($game_id, array(50,64,47,48))){
			return;
		}
		...
		if(in_array($this->game_id, $this->game_lists)){
	...
	protected function getArguments()
	{
		return array(
			...
			array('test', InputArgument::REQUIRED, 'test'),
		);
	}
