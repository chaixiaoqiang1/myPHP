...
class ImportRingsLogIntoDB extends Command {
	const TABLE_NAME = 'log_rings';
    const FILE_KEY = 'RingsLog';
    const PREFIX_LOG_FILE = '/home/game/trans/data/';
    const LOG_BACKUP = '/data/logbackup/';
    const FLAG_TYPE = 'rings';
    private $log_file = '';
    private $log_file_bak = '';
    private $db_name = '';
    private $rings_files = array();
    private $db = '';
	 ...
	 * @var string
	 */
	protected $description = 'Import Rings Log.';
	...
	private function loadRingsFile()
	{
	    try
	    {
	        $result = $this->db->select("show tables like 'log_rings'");//后续需要修改
	        if(!$result){
	    ...
	    Log::info('Import ' . $this->db_name . ' Rings Log');
	    try
	    {
	    ...
	    if (! $files)
	    {
	        Log::info($this->db_name . ' Rings Log Not Found');
	        return;
	    }
	    $this->rings_files = array();
	    foreach ($files as $v)
	    {
	        ...
	        if (strpos($v, self::FILE_KEY) !== false)
	        {
	            ...
	            $this->rings_files[$tmp[1]] = $v;
	        }
	    }
	    ksort($this->rings_files);
	    $this->readLog(); 
	...
	private function createTable($db_name){
	    ...
	    $sql="create table log_rings(
	        ...
	        `googs_id` int(11) unsigned NOT NULL, 
	        `start_lev` int(10) unsigned NOT NULL,
	        `end_lev` int(10) unsigned NOT NULL,
	        `start_rings_exp` int(11) NOT NULL,
	        `end_rings_exp` int(10) NOT NULL,
	...
	{
	    while (! empty($this->rings_files))
	    {
	        $pos = - 1;
	        $v = array_shift($this->rings_files);
	        Log::info('Import ' . $v);
