...
class ImportMergegemLogIntoDB extends Command {

	const TABLE_NAME = 'log_mergegem';
	const FILE_KEY = 'MergeGemLog';
	const PREFIX_LOG_FILE = '/home/game/trans/data/';
	const LOG_BACKUP = '/data/logbackup/';
	const FLAG_TYPE = 'mergegem';
	...
	private function createTable(){
	    ...
	    $sql="CREATE TABLE IF NOT EXISTS `log_mergegem`(
	...
	protected $name = 'mergegem:import';
	 ...
	 * @var string
	 */
	protected $description = 'Import Mergegem Log.';
	...
	private function loadGmaeFile()
	{
	    ...
	    try
	    {
	        $result = $this->db->select("show tables like 'log_mergegem'");
	        if(!$result){
	            $this->createTable();
	        }  
	    }
	    ...
	    Log::info('Import ' . $this->db_name . ' Mergegem Log');
	    try
	    {
	    ...
	    if (! $files)
	    {
	        Log::info($this->db_name . ' Mergegem Log Not Found');
	        return;
	    }
