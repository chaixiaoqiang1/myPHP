	...
	function crossServerTournamentController($scope, $http, alertService, $filter) {
		...
		$scope.lookup = function(url) {
			...
			$http({
	            ...
	            for (var i = 0; i < len; i++) {
	                if (result[i]['status'] == 'ok') {
	                    for (var j = 0; j < result[i].msg.length; j++) {
		...
		<div class="col-xs-8">
			...
			<div class="form-group" style="height: 40px;">
				<div class="input-group">
					...
					<i class="glyphicon glyphicon-calendar"></i> <span><i><?php echo Lang::get('serverapi.tournament_cross_server_time') ?></i></span>
				</div>

			</div>

			<div class="form-group">
				<select class="form-control" name="server_id2" ng-model="formData.server_id2"
					ng-init="formData.server_id2=0" multiple="multiple"
			...
			<div class="form-group" style="height: 40px;">
					...
					<input type='button' class="btn btn-primary"
						value="<?php echo Lang::get('serverapi.tournament_cross_open') ?>"
						ng-click="process('/game-server-api/tournament/cross-server')" />
				
					<input type='button' class="btn btn-primary"
						value="<?php echo Lang::get('serverapi.tournament_cross_connect') ?>"
						ng-click="process('/game-server-api/tournament/cross-server/update')" style="margin-left:5px"/>
				
					<input type='button' class="btn btn-primary"
						value="<?php echo Lang::get('serverapi.tournament_cross_signup') ?>"
						ng-click="process('/game-server-api/tournament/cross-server/signup')" style="margin-left:5px"/>
				
					<input type='button' class="btn btn-primary"
						value="<?php echo Lang::get('serverapi.tournament_lookup') ?>"
						ng-click="lookup('/game-server-api/tournament/cross-server/lookup')" style="margin-left:5px"/>
				
					<input type='button' class="btn btn-primary"
						value="<?php echo Lang::get('serverapi.tournament_look') ?>"
						ng-click="look('/game-server-api/tournament/cross-server/look')" style="margin-left:5px"/>
			...
			<div class="col-md-4" style="padding: 0">
				<input type="button" class="btn btn-danger"
					...
					ng-click="process('/game-server-api/tournament/cross-server/close')" />
			</div>
		</div>
	</div>
