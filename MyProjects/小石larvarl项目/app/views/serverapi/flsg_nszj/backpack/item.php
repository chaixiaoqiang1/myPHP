<script>
	function ItemController($scope, $http, alertService, $filter) {
		...
		$scope.process = function(newPage) {
			...
			$http({
				'method' : 'post',
				'url'	 : '/game-server-api/user/item?page='+newPage,
			...
			<form method="post" ng-submit="process()" onsubmit="return false;">
				...
				<div class="col-md-6">
					<select class="form-control" name="item_id" ng-model="formData.item_id" ng-init="formData.item_id=0">
						...
						<option value="<?php echo isset($v->id)? $v->id : $v->Id;?>"><?php echo (isset($v->id)? $v->id : $v->Id) . ':' . (isset($v->name)? $v->name : $v->Name);?></option>
						<?php } ?>		
					</select>
				</div>
				...
				<div class="col-md-6" style="padding-top: 5px">
					<input type="text" name="item_name" ng-model="formData.item_name" class="form-control" placeholder="<?php echo Lang::get('serverapi.enter_item_name') ?>"/>
				</div>
			<br>
			<br>
			<div class="col-md-6" style="padding-top: 5px">
		...
		<table class="table table-striped">
			<thead>
				<tr class="info">
					...
					<td><b><?php echo Lang::get('slave.num');?></b></td>
