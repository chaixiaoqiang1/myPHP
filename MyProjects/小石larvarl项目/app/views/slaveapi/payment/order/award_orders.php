<script>
	function getRecordOrdersController($scope, $filter, $modal, $http, alertService) {
		...
		$scope.processFrom = function(type, newPage) {
			...
			$scope.formData.order_type = 'award';
			...
			$http({
				'method' : 'post',
				'url'	 : '/slave-api/payment/order/award<?php echo ($is_gm ? '/gm' : '') ?>',
...
<div class="col-xs-12" ng-controller="getRecordOrdersController">
	...
	<div class="col-xs-12" ng-if="show == 1">
		<?php if(!$is_gm){ ?>
		<p style="color:red;font-size:15px"><b>为了方便发奖励，点击order_id将打开对应游戏下的发奖励功能</b></p>
		<?php } ?>
		<table class="table table-striped">
			<thead>
				<tr class="info">
					...
					<td><b><?php echo Lang::get('slave.award_content'); ?></b></td>
			...
			<tbody>
				<tr ng-repeat="o in orders">
					<?php if(!$is_gm){ ?>
					<td><a href="<?php echo $award_link; ?>" target="award_link"><button class="btn btn-primary">{{o.order_id}}</button></a></td>
					<?php }else{ ?>
					<td>{{o.order_id}}</td>
					<?php } ?>
					<td>{{o.order_sn}}</td>
					...
					<td>{{o.server_name}}</td>
					<td>{{o.pay_user_id}}</td>
					<td style="color:red">{{o.reason}}</td>
					...
					<?php if(!$is_gm){ ?>
						...
						<td ng-if="o.deal_time != '-'">{{o.result}}</td>
					<?php }else{ ?>
					...
					<?php if($is_gm){ ?>
						...
						<td ng-if="o.is_done==0 && o.deal_time != '-'"><?php echo Lang::get('slave.order_not_done'); ?><button class="btn btn-primary" ng-click="FinishOrder(o.id)"><?php echo Lang::get('slave.done_order'); ?></button>
